...
export function conformToMask(
      ...
      if (rawValueArr.length > 0) {
        ...
        while (rawValueArr.length > 0) {
          ...
          if (rawValueChar === placeholderChar && suppressGuide !== true) {
            ...
            // We know it's not a placeholderChar so can assume its a Regexp
  ...
  if (suppressGuide && isAddition === false) {
    ...
    if (indexOfLastFilledPlaceholderChar !== null) {
      // If there are cursorTraps, and the char to the right last filled placeholder is
      // in that trap, include it in conformedValue
      let rightCharIndex = indexOfLastFilledPlaceholderChar + 1;
      let trappedChar =
        (cursorTrapIndexes.length === 2 &&
          rightCharIndex > cursorTrapIndexes[0]) ||
        rightCharIndex <= cursorTrapIndexes[0]
          ? 1
          : 0;
      ...
      conformedValue = conformedValue.substr(
        0,
        indexOfLastFilledPlaceholderChar + 1 + trappedChar
      );
    } else {
